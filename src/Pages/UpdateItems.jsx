...
function UpdateItems() {
   ...
   useEffect (()=>{
    const fetchAdditem =async()=>{
        const addeditemId=params.addeditemId
        const res=await fetch(`/api/add/get/${addeditemId}`)
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        setError(false);
        const res = await fetch(`/api/add/update/${params.addeditemId}`, { method: 'POST', headers: {'Content-Type': 'application/json' },
          body: JSON.stringify({ ...formData, userRef:currentUser._id })
        });
        const data = await res.json()
        if (data.success === false) {
          setError(data.message)
          toast.warning("property updated failed")
        }
  ...
  return (
   ...
   <div className='bg-slate-700' style={{height:"120vh"}}>
      ...
      <div className='border bg-white my-5 p-5 rounded-lg  bg-blend-saturation'>
        <h1 className='text-3xl font-bold text-center my-4'> UPDATE PROPERTY </h1>
        <form className='flex flex-col sm:flex-row  gap-4 p-2' onSubmit={handleSubmit}>
            ...
            </div>
          {
           formData.imageurls.length > 0 && formData.imageurls.map((url,index)=>(
            ...
            <button className='p-3 bg-green-600 rounded-lg hover:opacity-90'>UPDATE </button>
          </div>
        </form>
        </div>
      </main>
